...
class Soundcloud:
    ...
    def __init__(self, settings: Settings):
        ...
        # Log what we have
        if self.client_id:
            logger.info(f"Found SoundCloud CLIENT_ID: {self.client_id[:10]}... (length: {len(self.client_id)})")
        else:
            logger.warning("SOUNDCLOUD_CLIENT_ID is not set in .env file")
        
        if self.client_secret:
            logger.info(f"Found SoundCloud CLIENT_SECRET: {self.client_secret[:10]}... (length: {len(self.client_secret)})")
        
        if self.access_token:
            logger.info(f"Found SoundCloud ACCESS_TOKEN: {self.access_token[:10]}... (length: {len(self.access_token)})")
        else:
            logger.info("SOUNDCLOUD_ACCESS_TOKEN not set - will use Client Credentials grant for public resources")
    ...
    def _get_client_credentials_token(self) -> bool:
        ...
        if not client_secret:
            logger.error("SOUNDCLOUD_CLIENT_SECRET is not set. Cannot get client credentials token.")
            return False
        
        logger.info("Getting access token via Client Credentials grant (for public resources)...")
        ...
        try:
            ...
            logger.error(f"Failed to get client credentials token: HTTP {e.response.status_code}")
    ...
    def _refresh_access_token(self) -> bool:
        ...
        if not refresh_token:
            logger.warning("SOUNDCLOUD_REFRESH_TOKEN is not set. Trying Client Credentials grant instead...")
            return self._get_client_credentials_token()
        
        if not client_secret:
            logger.error("SOUNDCLOUD_CLIENT_SECRET is not set. Cannot refresh access token.")
            return False
        ...
        try:
            ...
            if new_access_token:
                ...
                logger.debug(f"Old token: {old_token_preview}... -> New token: {new_access_token[:20]}...")
                ...
                logger.warning("Refresh failed: No access token in response. Trying Client Credentials...")
                return self._get_client_credentials_token()
                
        except requests.exceptions.HTTPError as e:
            logger.warning(f"Failed to refresh token: HTTP {e.response.status_code}. Trying Client Credentials...")
            return self._get_client_credentials_token()
        ...
        try:
            ...
            if e.response.status_code == 401:
                ...
                if self._refresh_access_token():
                    ...
                    playlist_data = response.json()
                    return playlist_data.get("tracks", [])
            logger.error(f"Failed to get playlist tracks: HTTP {e.response.status_code}")
            logger.error(f"Response: {e.response.text}")
            raise
            ...
            if e.response.status_code == 401:
                ...
                if self._refresh_access_token():
                    ...
                    if stream_url:
                        # Don't append client_id - use Authorization header instead
                        return stream_url
            logger.error(f"Failed to get track stream URL: HTTP {e.response.status_code}")
            logger.error(f"Response: {e.response.text}")
            return None
    ...
    def download_track(self, track_data: dict) -> None:
        ...
        if track_id:
            pattern_with_id = os.path.join(self.output_folder, f"{base_filename} [{track_id}]*.mp3")
            existing_files = glob.glob(pattern_with_id)
            if existing_files:
                logger.info(f"  Track already exists (same ID): {os.path.basename(existing_files[0])}")
                return
            filename = os.path.join(self.output_folder, f"{base_filename} [{track_id}].mp3")
        else:
        ...
        while os.path.exists(filename):
            if track_id:
                filename = os.path.join(self.output_folder, f"{base_filename} [{track_id}] ({counter}).mp3")
            else:
                filename = os.path.join(self.output_folder, f"{base_filename} ({counter}).mp3")
            counter += 1
        ...
        try:
            ...
            logger.error(f"  Failed to download {title}. HTTP {e.response.status_code}: {error_msg}")
        ...
        for i, url in enumerate(self.urls_to_download):
            ...
            try:
                ...
                if resource_type == "playlist":
                    ...
                    logger.info(f"Playlist found: '{playlist_title}' (ID: {playlist_id})")
