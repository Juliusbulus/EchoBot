...
def main():
    ...
    if not folder_url:
        ...
        logger.error("Please add GOOGLE_DRIVE_FOLDER_URL=https://drive.google.com/drive/folders/YOUR_FOLDER_ID?usp=sharing to your .env file")
        return
    ...
    try:
        ...
        if skipped_count > 0:
            logger.info(f"⏭️  Skipping {skipped_count} file(s) that already exist locally")
        
        if not files_to_download_filtered:
            logger.info("✅ All files already exist locally. No downloads needed.")
            return
        
        logger.info(f"📥 Downloading {len(files_to_download_filtered)} missing file(s)...")
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt > 0:
                        ...
                        logger.info(f"Retrying download of {file_name} (attempt {attempt + 1}/{max_retries})... Waiting {delay}s before retry...")
                        time.sleep(delay)
                    else:
                    ...
                    if attempt == max_retries - 1:
                        # Last attempt failed
                        logger.error(f"Failed to download {file_name} (ID: {file_id}) after {max_retries} attempts: {e}")
        ...
        if failed_files:
            ...
            for file_name, file_id in failed_files:
                ...
                logger.warning(f"    Manual download URL: https://drive.google.com/uc?id={file_id}")
            logger.warning("\n💡 Solutions:")
            logger.warning("  1. Check file permissions: Right-click file → Share → 'Anyone with the link'")
